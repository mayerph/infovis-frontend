...
*/
if (
    typeof generator_embed_path == 'undefined' ||
    typeof generator_embed_path == 'undefined'
) {
    var generator_embed_path = getScriptPath(
...
function getLinkAndIframe() {
    ...
    if (e_source.value.match('docs.google.com')) {
        source_key = VMM.Timeline.DataObj.model.googlespreadsheet.extractSpreadsheetKey(
            e_source.value
        )
    } else {
        if (e_source.value == '') {
    ...
    if (parseInt(e_startzoomadjust.value, 10) > 0) {
        wp +=
            "start_zoom_adjust='" + parseInt(e_startzoomadjust.value, 10) + "' "
    }
    wp += ']'
    theobj.wordpress = wp
