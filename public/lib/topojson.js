!(function() {
            ...
            if ((f = fragmentByEnd[start])) {
                ...
                if ((g = fragmentByEnd[start])) {
                    ...
                    var gf = g
                        ...
                        .reverse()
                        .concat(f)
                    fragmentByStart[(gf.start = g.end)] = fragmentByEnd[
                        (gf.end = f.end)
                    ] = gf
                } else {
                    fragmentByStart[f.start] = fragmentByEnd[f.end] = f
                ...
                f = [i]
                fragmentByStart[(f.start = start)] = fragmentByEnd[
                    (f.end = end)
                ] = f
            }
        })
        ...
        if (arguments.length > 1) {
            ...
            geomsByArc.forEach(
                arguments.length < 3
                    ...
                    : function(geoms, i) {
                          if (filter(geoms[0], geoms[geoms.length - 1]))
                              arcs.push(i)
                      }
            )
        } else {
    ...
    function object(topology, o) {
        ...
        function arc(i, points) {
            ...
            for (
                var a = arcs[i < 0 ? ~i : i], k = 0, n = a.length, p;
                k < n;
                ++k
            ) {
        ...
        function geometry(o) {
            ...
            return t === 'GeometryCollection'
                ? { type: t, geometries: o.geometries.map(geometry) }
                : t in geometryType
                ? { type: t, coordinates: geometryType[t](o) }
                : null
        }
